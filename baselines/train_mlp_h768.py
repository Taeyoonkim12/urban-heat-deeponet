"""Baseline: single-path vanilla MLP (paper Table 4/6, 'MLP').

Same 6-D input information as M1 ([RH, T_amb, hour, x, y, z]) and the
same training protocol, but a plain residual MLP with hidden width 768
(parameter-matched to M1) instead of the branch/trunk operator split.
...
def main():
    ...
    # Same input pipeline as M1: coordinates only, no d_BP/categories.
    spec = MODEL_SPECS['m1']
    ...
    evaluate(model, spec, test_cases, scalers, args.out, dataset_cls,
             'MLP-vanilla 6D (h768)', device=device, log=log)
