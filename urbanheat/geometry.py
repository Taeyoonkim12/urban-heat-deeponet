...
def compute_dbp(query_xyz, building_tree, building_z):
    """Signed building-proximity feature d_BP (paper Sec. 2.3.2).

    Note: throughout the pipeline this feature is stored under the key
    'sdf' (its working name during development).

    Points within a 1 m horizontal buffer of the nearest building point
    are treated as on/inside the building and get a negative value
    -|dz| - 0.1; all other points get the (positive) distance to the
    nearest building point.
    """
